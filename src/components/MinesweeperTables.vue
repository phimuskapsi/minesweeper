          ...
          <v-list-item two-line>
            <v-list-item-content>
              ...
              <v-list-item-subtitle
                >Best Time: {{ gameData.highscores.time }}</v-list-item-subtitle
              >
              <v-list-item-subtitle
                >Timer: {{ gameData.time }}</v-list-item-subtitle
              >
            </v-list-item-content>
          </v-list-item>
          <v-card-text>
            <v-row align="center">
              <v-col cols="12">
                <v-simple-table
                  v-if="
                    gameData.grid.length > 0 && gameData.guessGrid.length > 0
                  "
                  class="game-table"
                >
                  <template v-slot:default>
                    <tbody>
                      <tr
                        v-for="(gridRow, rindex) in gameData.grid"
                        :key="rindex + '-tr'"
                      >
                        <td
                          v-for="(gridCol, cindex) in gridRow"
                          :key="rindex + '-tr-' + cindex + '-td'"
                          class="mine-button"
                          v-on:click="clickCell(cindex, rindex)"
                          v-on:click.right.prevent="markCell(cindex, rindex)"
                          v-bind:class="
                            gameData.grid[rindex][cindex].visible ||
                            gameData.grid[rindex][cindex].exploded
                              ? gameData.grid[rindex][cindex].exploded
                                ? 'pressed dark-red'
                                : 'pressed'
                              : ''
                        ...
                        >
                          <span
                            ...
                            v-if="
                              gameData.grid[rindex][cindex].visible &&
                                gameData.grid[rindex][cindex].value > 0
                            "
                          >
                            {{ gameData.grid[rindex][cindex].value }}
                          </span>
                          <span
                            ...
                            v-if="
                              !running &&
                                gameData.grid[rindex][cindex].wrong &&
                                !gameData.grid[rindex][cindex].bomb
                            "
                          >
                            X
                          </span>
                          ...
                          <v-icon
                            ...
                            v-if="
                              (!running &&
                                gameData.guessGrid[rindex][cindex].flag === 1 &&
                                gameData.grid[rindex][cindex].wrong) ||
                                (running &&
                                  gameData.guessGrid[rindex][cindex].flag ===
                                    1 &&
                                  !gameData.grid[rindex][cindex].visible)
                            "
                            >mdi-flag</v-icon
                          ...
                          <v-icon
                            ...
                            v-if="
                              running &&
                                gameData.guessGrid[rindex][cindex].flag === 2 &&
                                !gameData.grid[rindex][cindex].visible
                            "
            ...
            <v-row align="center">
              <v-col cols="4" offset="4">
                <v-btn color="primary" v-if="!running" v-on:click="startGame"
                  >Start Game</v-btn
                >
                <v-btn color="orange" v-if="running" v-on:click="solveGame"
                  >Solve</v-btn
                >
              </v-col>
            </v-row>
      ...
      for (let ty = -1; ty < 2; ty++) {
        for (let tx = -1; tx < 2; tx++) {
          ...
          if (
            testY < 0 ||
            testX < 0 ||
            testY >= this.gameData.size ||
            testX >= this.gameData.size
          )
            continue;
